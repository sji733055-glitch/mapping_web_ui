(function () {
  ...
  function describePreview(meta, grid) {
    ...
    if (meta.heightMode==="ground") {
      parts.push(Number.isFinite(meta.groundTilt)?`地面倾斜 ${meta.groundTilt.toFixed(2)}° 已校正`:"已校正倾斜地面");
      if (Number.isFinite(meta.groundLocalAnchors)) parts.push(`局部地面 ${formatNumber(meta.groundLocalAnchors)} 格`);
    }
  ...
  async function previewPcdMap() {
    ...
    try {
      ...
      const meta={
        ...
        groundLocalAnchors:readPreviewHeader(headers,"X-Map-Ground-Local-Anchors"),
